...
DROP POLICY IF EXISTS "Allow all customers operations" ON customers;
DROP POLICY IF EXISTS "Allow all bookings operations" ON bookings;
DROP POLICY IF EXISTS "Allow all villas operations" ON villas;
DROP POLICY IF EXISTS "Allow all settings operations" ON site_settings;
DROP POLICY IF EXISTS "Allow all inquiries operations" ON contact_inquiries;
DROP POLICY IF EXISTS "Allow all media operations" ON media_files;

-- Create policies
CREATE POLICY "Public can view active tours" ON tours FOR SELECT USING (is_active = TRUE);
CREATE POLICY "Public can view active villas" ON villas FOR SELECT USING (is_active = TRUE);
